...
namespace FlightPatternDetection.Controllers
{
    ...
    public class NavDbController : ControllerBase
    {
        ...
        [HttpGet("getLat")]
        public IEnumerable<EWayPoint> getLatPoints(double lat, double lng)
        {
            var s = Stopwatch.StartNew();


            var latPoints = _navDbManager.Waypoints.FindAll(x => ((x.Latitude + 1) >= lat && lat >= (x.Latitude - 1)) && (x.Longitude + 1 >= lng && lng >= (x.Longitude))).ToList();
            s.Stop();
            _logger.LogDebug($"Found {latPoints.Count} points in {s.Elapsed}");
